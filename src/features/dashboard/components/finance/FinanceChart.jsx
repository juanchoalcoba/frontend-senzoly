...
export default function FinanceChart({ data = [], grouping = 'daily', onGroupingChange, loading }) {
  ...
  const maxVal = Math.max(...data.map((d) => Math.max(d.grossTotal, d.netTotal, d.payoutTotal)), 1);
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm mb-6">
      ...
      {data.length === 0 ? (
        <div className="h-64 flex flex-col items-center justify-center bg-slate-50/60 rounded-2xl border border-dashed border-slate-200 text-center p-6">
          ...
          <p className="text-xs text-slate-400 mt-1">Los movimientos aparecerán automáticamente al completar turnos.</p>
        </div>
      ) : (
        <div className="relative">
          <div className="h-64 flex items-end gap-2 sm:gap-4 pt-8 pb-6 px-2 overflow-x-auto border-b border-slate-200 scrollbar-thin">
            {data.map((item, index) => {
              const grossHeight = Math.max((item.grossTotal / maxVal) * 100, 4);
              const netHeight = Math.max((item.netTotal / maxVal) * 100, 4);
              const payoutHeight = Math.max((item.payoutTotal / maxVal) * 100, 4);

              return (
                <div
                  key={index}
                  className="flex-1 min-w-[3rem] max-w-[5rem] flex flex-col items-center group relative cursor-pointer"
                  onMouseEnter={() => setActiveHover(item)}
                  onMouseLeave={() => setActiveHover(null)}
                >
                  {/* Tooltip Hover */}
                  {activeHover === item && (
                    <div className="absolute bottom-full mb-2 z-20 bg-slate-900 text-white text-xs rounded-xl p-3 shadow-xl whitespace-nowrap border border-slate-800 pointer-events-none transition-all">
                      ...
                      <p className="text-orange-400">Neto: {formatMoney(item.netTotal)}</p>
                  ...
                  <div className="w-full h-full flex items-end justify-center gap-1">
                    ...
                    <div
                      ...
                      className="w-1/3 bg-orange-500 group-hover:bg-orange-400 rounded-t transition-all duration-300"
                    ></div>
                    <div
